...
export const columnExtensionFile: IColumnExtension = {
  ...
  create: async (
    ...
    const types: string[] =
      context.column.properties != null && context.column.properties.types != null
        ? context.column.properties.types
        : ["DEFAULT"];
    types.sort();
  ...
  update: async (
    ...
    if (
      ...
      result.commands.push({
        sqls: [
          `ALTER TABLE ${getPgRegClass(table)} ALTER COLUMN "${column.name}" SET DEFAULT ${
        ...
        autoSchemaFixes: [
          {
            ...
            message: "FileStorage Expression not reproducable TODO: better message",
          },
        ],
      });
    }
    ...
    if (currentlyNullable === true && (column.properties == null || column.properties.nullable !== true)) {
      ...
      if (column.properties != null && column.properties.defaultExpression != null) {
        sqls.push(
          `UPDATE ${getPgRegClass(table)} SET "${column.name}" = ${column.properties.defaultExpression} WHERE "${
            column.name
          }" IS NULL;`
        );
      }
