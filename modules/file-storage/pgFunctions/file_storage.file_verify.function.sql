...
CREATE OR REPLACE FUNCTION _file_storage.file_verify(i_file_id uuid) RETURNS void AS $$
DECLARE
    v_user_id uuid;
...
BEGIN
    ...
    v_query := $tok$SELECT "ownerUserId", "deletedAt", "verifiedAt", "type" FROM _file_storage."Files" WHERE id = %L$tok$;
    ...
    v_query := $tok$UPDATE _file_storage."Files" SET "verifiedAt"=now() WHERE id = %L$tok$;
