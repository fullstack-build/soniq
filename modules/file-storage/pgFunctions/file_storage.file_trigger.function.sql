...
CREATE OR REPLACE FUNCTION _file_storage.file_trigger() RETURNS trigger AS $$
    ...
    function validateFile(fileName, entityId, types) {
        ...
        var plan = plv8.prepare( 'SELECT _file_storage."file_validate"($1, $2, $3::TEXT[]);', ['TEXT', 'uuid', 'TEXT[]'] );
    ...
    function invalidateFile(fileName, entityId) {
        ...
        var plan = plv8.prepare( 'SELECT _file_storage."file_invalidate"($1, $2);', ['uuid', 'uuid'] );
        var rows = plan.execute( [fileId, entityId] );
    }

    var tableId = TG_TABLE_SCHEMA + '.' + TG_TABLE_NAME;

    var plan = plv8.prepare( 'SELECT json_agg(row_to_json(fc)) AS "fields" FROM _file_storage."Columns" AS fc WHERE "schemaName" = $1 AND "tableName" = $2;', ['text', 'text'] );
