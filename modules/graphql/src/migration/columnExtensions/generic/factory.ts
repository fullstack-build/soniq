...
import { IColumnExtension, IColumnExtensionContext, IQueryFieldData, IMutationFieldData } from "../IColumnExtension";
...
export function getDefaultAndNotNullChange(
  table: IDbTable,
  column: IDbColumn,
  ...
  const currentDefaultExpression: string | null = columnInfo.column_default;
...
export function createGenericColumnExtension(
  ...
  const getPgColumnName: (context: IColumnExtensionContext) => string = (context: IColumnExtensionContext): string => {
  ...
  return {
    ...
    create: async (context: IColumnExtensionContext): Promise<IGqlMigrationResult> => {
      let sql: string = `ALTER TABLE ${getPgRegClass(context.table)} ADD COLUMN "${getPgColumnName(context)}" ${
        fixedGenericTypes.pgDataType
      }`;
