...
export class DbSchemaValidator {
  ...
  private validateColumn(column: IDbColumn) {
    const errors = [];
  ...
  private validateTable(table: IDbTable) {
    const errors = [];
    ...
    if (table.columns == null || !Array.isArray(table.columns)) {
      ...
      table.columns.forEach((column) => {
        const validationErrors = this.validateColumn(column);
  ...
  private validateDbSchema(dbSchema: IDbSchema): IMigrationError[] {
    const errors = [];
    // Check Schemas
    if (dbSchema.schemas == null || !Array.isArray(dbSchema.schemas)) {
      errors.push(`DbSchema.schemas: Must be an array of strings.`);
    } else {
      dbSchema.schemas.forEach((schemaName) => {
        this.validateSchema(schemaName).forEach((error) => {
          errors.push(error);
        });
      });
    }
  ...
  public validate(dbSchema: IDbSchema): IMigrationError[] {
    return this.validateDbSchema(dbSchema);
  }
}
