...
const expectedQueryBuildObject: IQueryBuildOject = {
  sql: `SELECT (SELECT COALESCE(json_agg(row_to_json("_local_0_")), '[]'::json) FROM (SELECT "_local_1_"."id" "id", (SELECT COALESCE(json_agg(row_to_json("_local_2_")), '[]'::json) FROM (SELECT "_local_3_"."id" "id" FROM "_graphql"."MOVIE_READ_PUBLIC" AS "_local_3_" WHERE "_local_3_"."directorId" = "_local_1_"."id") "_local_2_") "movies" FROM "_graphql"."DIRECTOR_READ_PUBLIC" AS "_local_1_") "_local_0_") "directors";`,
  values: [],
  queryName: "directors",
  authRequired: false,
