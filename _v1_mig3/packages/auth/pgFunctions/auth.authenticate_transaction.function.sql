...
BEGIN
    ...
    SELECT value INTO v_transaction_token_timestamp FROM _auth."Settings" WHERE key = 'transaction_token_timestamp';
    SELECT value INTO v_transaction_token_max_age_in_seconds FROM _auth."Settings" WHERE key = 'transaction_token_max_age_in_seconds';
    ...
    IF (v_timestamp_sec - v_transaction_token_timestamp) > v_transaction_token_max_age_in_seconds THEN
        ...
        UPDATE _auth."Settings" SET "value"=crypt(encode(gen_random_bytes(64), 'hex'), gen_salt('bf', 4)) WHERE "key"='transaction_token_secret';

        -- Set transaction_token_secret to the current time to flag the secret as updated.
        UPDATE _auth."Settings" SET "value"=round(extract(epoch from now()))::text WHERE "key"='transaction_token_timestamp';
