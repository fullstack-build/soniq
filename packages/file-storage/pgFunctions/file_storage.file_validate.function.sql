...
CREATE OR REPLACE FUNCTION _file_storage.file_validate(i_file_name TEXT, i_entity_id uuid, i_types TEXT[]) RETURNS void AS $$
DECLARE
    v_user_id uuid;
...
BEGIN
    ...
    v_query := $tok$UPDATE _file_storage."Files" SET "entityId"=%L WHERE id = %L$tok$;
