  ...
  Object.values(filteredViews).forEach((view) => {
    ...
    tableView.fields = tableView.fields.filter((field) => {
      ...
      if (!isIncluded && view.type === 'READ') {
        Object.values(view.fields).forEach((viewFieldName: any) => {
    ...
    Object.values(tableView.fields).forEach((field: any) => {
      const fieldName = field.name.value;
    ...
    Object.values(view.expressions).forEach((expression: any) => {
