...
export class Auth {
  ...
  constructor(
    ...
    this.schemaBuilder.addExtension(
      getParser(
        (key, value) => {
  ...
  private async boot() {
    ...
    Object.keys(this.authConfig.oAuth.providers).forEach((key) => {
      ...
      authRouter.get(
        callbackPath,
        errorCatcher,
        passport.authenticate(provider.name, { failureRedirect: "/auth/oAuthFailure", session: false }),
        (ctx) => {
    ...
    if (mutation.extensions.auth === "REGISTER_USER_MUTATION") {
      ...
      if (this.isPrivacyAgreementCheckActive() === true) {
        ...
        if (
          ...
          throw new Error(
            `The privacy-fields ('${privacyAgreementAcceptedAtInUTC}',` +
  ...
  public async login(username, tenant, password, authToken, clientIdentifier) {
    ...
    try {
      ...
      const loginResult = await dbClient.query("SELECT _meta.login($1, $2, $3, $4) AS payload", [
        data.userId,
        provider,
        pwData.hash,
        clientIdentifier
