...
CREATE OR REPLACE FUNCTION _auth.proof_auth_factor(i_auth_factor jsonb) RETURNS void AS $$
DECLARE
    v_is_admin BOOLEAN;
    v_hash_secret TEXT;    
...
BEGIN
    ...
    v_query := $tok$ UPDATE "_auth"."AuthFactor" SET "proofedAt" = COALESCE("proofedAt", timezone('UTC'::text, now())) WHERE "id" = %L; $tok$;
