...
CREATE OR REPLACE FUNCTION _meta.create_user_authentication(i_user_id TEXT, i_is_active boolean, i_login_provider_sets TEXT[], i_modify_provider_sets TEXT[], i_auth_factors jsonb) RETURNS jsonb AS $$
DECLARE
    v_is_admin BOOLEAN;
    v_hash_secret TEXT;    
...
BEGIN
    ...
    LOOP
        ...
        END IF;
     	...
     	v_query := $tok$ INSERT INTO "_meta"."AuthFactor"("userAuthenticationId", "provider", "meta", "hash", "communicationAddress") VALUES(%L, %L, %L, %L, %L) RETURNING "id"; $tok$;

        v_is_proofed := v_auth_factor->>'isProofed';
        IF v_is_proofed IS TRUE THEN
            v_query := $tok$ INSERT INTO "_meta"."AuthFactor"("userAuthenticationId", "provider", "meta", "hash", "communicationAddress", "proofedAt") VALUES(%L, %L, %L, %L, %L, timezone('UTC'::text, now())) RETURNING "id"; $tok$;
        END IF;
