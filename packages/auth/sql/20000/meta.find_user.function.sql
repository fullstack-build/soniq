...
CREATE OR REPLACE FUNCTION _meta.find_user(i_communication_address TEXT, i_tenant TEXT) RETURNS jsonb AS $$
DECLARE
    v_is_admin BOOLEAN;
    v_query TEXT;
...
BEGIN
    ...
    v_query := $tok$ SELECT (SELECT "id" FROM _meta."UserAuthentication" WHERE "id" = af."userAuthenticationId" AND _meta.get_tenant_by_userid("userId"::TEXT) = %L), "id" FROM "_meta"."AuthFactor" af WHERE "communicationAddress" = %L AND "deletedAt" IS NULL; $tok$;
