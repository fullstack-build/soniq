-- login function checks provided AuthFactors and returns a loginToken and refreshToken
...
          RAISE EXCEPTION 'Login failed! AuthFactor invalid.';
      END IF;
      
...
        RAISE EXCEPTION 'Login failed! Invalid AuthFactor set.';
      END IF;
      
      v_previous_user_authentication_id := v_user_authentication_id;
    ...
        v_validation_hash := crypt(encode(digest(v_auth_factor->>'hash' || v_hash_secret, 'sha256'), 'hex'), v_auth_factor_hash);
     ...
        RAISE EXCEPTION 'Login failed! AuthFactor hash not matching.';
        END IF;
        
...
      RAISE EXCEPTION 'Login failed! UserId not found.';
    END IF;

    IF v_user_authentication_active IS NOT TRUE THEN
      RAISE EXCEPTION 'Login failed! UserAuthentication inactive.';
    END IF;

    IF NOT v_provider_set = ANY(v_login_provider_sets) THEN
      RAISE EXCEPTION 'Login failed! Invalid ProviderSet.';
    END IF;
  
