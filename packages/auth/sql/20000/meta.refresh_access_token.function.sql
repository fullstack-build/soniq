...
CREATE OR REPLACE FUNCTION _meta.refresh_access_token(i_access_token TEXT, i_client_identifier TEXT, i_refresh_token TEXT) RETURNS jsonb AS $$
DECLARE
    v_is_admin BOOLEAN;
...
    v_any_result := _meta.invalidate_access_token(i_access_token);

    FOREACH v_auth_factor_id IN ARRAY v_auth_factor_ids
    LOOP
      v_query := $tok$ SELECT "hash", "provider" FROM "_meta"."AuthFactor" WHERE "id" = %L AND "deletedAt" IS NULL; $tok$;
