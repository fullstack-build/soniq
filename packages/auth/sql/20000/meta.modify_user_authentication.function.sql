...
CREATE OR REPLACE FUNCTION _meta.modify_user_authentication(i_auth_factors jsonb, i_is_active BOOLEAN, i_login_provider_sets TEXT[], i_modify_provider_sets TEXT[], i_new_auth_factors jsonb, i_remove_auth_factor_ids TEXT[]) RETURNS void AS $$
DECLARE
    v_is_admin BOOLEAN;
    v_hash_secret TEXT;    
...
    END IF;
    ...
    v_query := $tok$ UPDATE "_meta"."UserAuthentication" SET "isActive" = COALESCE(%L, "isActive"), "loginProviderSets" = COALESCE(%L, "loginProviderSets"), "modifyProviderSets" = COALESCE(%L, "modifyProviderSets") WHERE "id" = %L; $tok$;
    ...
            v_query := $tok$ UPDATE "_meta"."AuthFactor" SET "deletedAt" = timezone('UTC'::text, now()) WHERE "id" = %L; $tok$;
    ...
    IF i_new_auth_factors IS NOT NULL THEN
        ...
            v_protected_hash := crypt(encode(digest(v_auth_factor->>'hash' || v_hash_secret, 'sha256'), 'hex'), gen_salt('bf', v_hash_bf_iter_count));
            ...
            v_query := $tok$ SELECT count(*) FROM _meta."AuthFactor" af WHERE "deletedAt" IS NULL AND af."communicationAddress" IS NOT NULL AND af."communicationAddress" = %L AND _meta.get_tenant_by_userid((SELECT "userId" FROM "_meta"."UserAuthentication" WHERE id = af."userAuthenticationId")::TEXT) = _meta.get_tenant_by_userid(%L); $tok$;
        ...
            v_query := $tok$ INSERT INTO "_meta"."AuthFactor"("userAuthenticationId", "provider", "meta", "hash", "communicationAddress") VALUES(%L, %L, %L, %L, %L) $tok$;
            
            v_is_proofed := v_auth_factor->>'isProofed';
            IF v_is_proofed IS TRUE THEN
                v_query := $tok$ INSERT INTO "_meta"."AuthFactor"("userAuthenticationId", "provider", "meta", "hash", "communicationAddress", "proofedAt") VALUES(%L, %L, %L, %L, %L, timezone('UTC'::text, now())) $tok$;
            END IF;
