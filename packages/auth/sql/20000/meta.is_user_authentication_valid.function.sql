...
CREATE OR REPLACE FUNCTION _meta.is_user_authentication_valid(i_user_authentication_id TEXT) RETURNS BOOLEAN AS $$
DECLARE
    v_is_admin BOOLEAN;
...
BEGIN
  ...
      END IF;
    ...
    v_query := $tok$ SELECT "loginProviderSets", "modifyProviderSets" FROM _meta."UserAuthentication" WHERE id = %L; $tok$;
    ...
      v_provider_set_array := regexp_split_to_array(v_provider_set, ':');
 	    ...
 	    v_provider_set_array_sorted := _meta.array_sort(v_provider_set_array);
