...
exports.parseGQlAstToDbMeta = (graphQlJsonSchema, directiveParser) => {
    const dbMeta = {
        version: 1.0,
        schemas: {},
        enums: {},
    ...
    // start parsing
    createGqlParser(directiveParser)(graphQlJsonSchema, dbMeta);
    // return copy instead of ref
    return _.cloneDeep(dbMeta);
};
// create context for parsers
function createGqlParser(directiveParser) {
    ...
    const GQL_JSON_PARSER = {
        ...
        NamedType: (gQlSchemaNode, dbMetaNode, refDbMeta, refDbMetaCurrentTable, refDbMetaCurrentTableColumn) => {
            ...
            switch (columnTypeLowerCase) {
                case 'id':
                    ...
                    gQlAstToDbMetaHelper_1.addConstraint('PRIMARY KEY', gQlSchemaNode, dbMetaNode, refDbMeta, refDbMetaCurrentTable, refDbMetaCurrentTableColumn);
                    break;
                case 'string':
                    dbMetaNode.type = 'varchar';
                    break;
        ...
        NonNullType: (gQlSchemaNode, dbMetaNode, refDbMeta, refDbMetaCurrentTable, refDbMetaCurrentTableColumn) => {
            // add new constraint
            gQlAstToDbMetaHelper_1.addConstraint('not_null', gQlSchemaNode, dbMetaNode, refDbMeta, refDbMetaCurrentTable, refDbMetaCurrentTableColumn);
            // parse sub type
