...
function runtimeParser(userSchema, views, expressions, dbObject) {
    const classification = classifyUserDefinitions_1.default(userSchema);
    const { document, dbViews, gQlTypes, queries, mutations, customFields } = createPublicSchema_1.default(classification, views, expressions, dbObject);
