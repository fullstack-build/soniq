...
exports.parseGraphQlJsonSchemaToDbMeta = (graphQlJsonSchema) => {
    const dbMeta = {
        version: 1.0,
        schemas: {},
        enums: {},
...
function parseGraphQlJsonNode(gQlSchemaNode, dbMetaNode, dbMeta, refDbMetaCurrentTable, refDbMetaCurrentTableColumn) {
...
const GQL_JSON_PARSER = {
    ...
    ObjectTypeDefinition: (gQlSchemaDocumentNode, dbMetaNode, refDbMeta, continueRecursively = true) => {
        ...
        Object.values(gQlSchemaDocumentNode).map((gQlSchemaDocumentNodeProperty) => {
            ...
            if (Array.isArray(gQlSchemaDocumentNodeProperty)) {
                Object.values(gQlSchemaDocumentNodeProperty).map((gQlSchemaDocumentSubnode) => {
                    // parse sub node
                    parseGraphQlJsonNode(gQlSchemaDocumentSubnode, refDbMetaCurrentTable, refDbMeta, refDbMetaCurrentTable);
                });
            }
        });
    },
    // parse EnumType
    EnumTypeDefinition: (gQlEnumTypeDefinitionNode, dbMetaNode, refDbMeta) => {
    ...
    Directive: (gQlDirectiveNode, dbMetaNode, refDbMeta, refDbMetaCurrentTable, refDbMetaCurrentTableColumn) => {
        ...
        switch ((directiveKind).toLowerCase()) {
            case 'table':
                ...
                break;
            case 'unique':
                addConstraint('UNIQUE', gQlDirectiveNode, dbMetaNode, refDbMeta, refDbMetaCurrentTable, refDbMetaCurrentTableColumn);
                break;
            case 'check':// native PG check constraint
                // iterate over all constraints
                gQlDirectiveNode.arguments.forEach((argument) => {
                    addConstraint('CHECK', argument, dbMetaNode, refDbMeta, refDbMetaCurrentTable, refDbMetaCurrentTableColumn);
                });
                break;
            case 'validate':// validate constraint
                // iterate over all constraints
                gQlDirectiveNode.arguments.forEach((argument) => {
                    addConstraint('validate', argument, dbMetaNode, refDbMeta, refDbMetaCurrentTable, refDbMetaCurrentTableColumn);
                });
                break;
    ...
    FieldDefinition: (gQlFieldDefinitionNode, dbMetaNode, refDbMeta, refDbMetaCurrentTable) => {
        ...
        if (_.get(gQlFieldDefinitionNode, 'directives[0].name.value') === 'relation') {
            // handle relation
            relationBuilderHelper(gQlFieldDefinitionNode, dbMetaNode, refDbMeta, refDbMetaCurrentTable);
        }
        else {
            ...
            Object.values(gQlFieldDefinitionNode).map((gQlSchemaFieldNodeProperty) => {
                if (typeof gQlSchemaFieldNodeProperty === 'object' &&
                    ...
                    Object.values(gQlSchemaFieldNodeProperty).map((gQlSchemaFieldSubnode) => {
                        // parse sub node
                        parseGraphQlJsonNode(gQlSchemaFieldSubnode, newField, refDbMeta, refDbMetaCurrentTable, newField);
                    });
                }
            });
    ...
    NamedType: (gQlSchemaNode, dbMetaNode, refDbMeta, refDbMetaCurrentTable, refDbMetaCurrentTableColumn) => {
        ...
        switch (columnTypeLowerCase) {
            case 'id':
                ...
                addConstraint('PRIMARY KEY', gQlSchemaNode, dbMetaNode, refDbMeta, refDbMetaCurrentTable, refDbMetaCurrentTableColumn);
                break;
            case 'string':
                dbMetaNode.type = 'varchar';
                break;
    ...
    NonNullType: (gQlSchemaNode, dbMetaNode, refDbMeta, refDbMetaCurrentTable, refDbMetaCurrentTableColumn) => {
        // add new constraint
        addConstraint('not_null', gQlSchemaNode, dbMetaNode, refDbMeta, refDbMetaCurrentTable, refDbMetaCurrentTableColumn);
        // parse sub type
        if (gQlSchemaNode.type != null) {
            const gQlSchemaTypeNode = gQlSchemaNode.type;
            parseGraphQlJsonNode(gQlSchemaTypeNode, dbMetaNode, refDbMeta, refDbMetaCurrentTable, refDbMetaCurrentTableColumn);
        }
    },
    // set list type
    ...
    // parse Argument
    Argument: (gQlNode, dbMetaNode, refDbMeta, refDbMetaCurrentTable, refDbMetaCurrentTableColumn) => {
...
function relationBuilderHelper(gQlDirectiveNode, dbMetaNode, refDbMeta, refDbMetaCurrentTable) {
    ...
    if (refDbMeta.schemas[relationSchemaName].tables[relationTableName] == null ||
        ...
        if (thisRelation == null || otherRelation == null) {
            process.stderr.write('GraphQL.parser.error.relation.too.many: ' + relationName +
