...
function setAuthValueForColumn(directiveKind,
                               gQlSchemaNode,
                               dbMetaNode,
                               refDbMeta,
                               refDbMetaCurrentTable,
      ...
      if (dbMetaNode.auth == null) {
        ...
        switch (directiveKindLowerCase) {
          case 'tenant':
            ...
            const columnMarkedTenant = Object.values(refDbMetaCurrentTable.columns).filter((column: any) => (column.auth && column.auth.isTenant));
